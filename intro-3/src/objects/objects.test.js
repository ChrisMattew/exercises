...
describe('hasValidProperty', () => {
  ...
  it('Returns true with a valid property - 1', () => {
    expect(
      hasValidProperty(object, (_, value) => Number.isFinite(value) && value < 5)
    ).toEqual(true)
  })

  it('Returns true with a valid property - 2', () => {
    expect(
      hasValidProperty(object, (key, value) => key === 'children' && value.length > 0)
    ).toEqual(true)
  })

  it('Returns false with no valid property', () => {
    expect(
      hasValidProperty(object, (_, value) => Number.isFinite(value) && value > 100)
    ).toEqual(false)
  })
})
...
describe.only('countTreeLeafNodes', () => {
